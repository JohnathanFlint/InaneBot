...
public class CTECTwitter
{
	...
	public void sendTweet(String textToTweet)
	{
		try
		{
			chatbotTwitter.updateStatus(
					"I Mr. Salochin just tweeted from my Java Chatbot program 2017! #APCSRocks @CTECNow Thanks @cscheerleader & @codyhenrichsen! @ChatbotCTEC");
		...
		for (int index = 0; index < tweetedWords.size(); index++)
		{
			...
			for (int searched = index + 1; searched < tweetedWords.size(); searched++)
			{
				if (tweetedWords.get(index).equalsIgnoreCase(tweetedWords.get(searched))
						&& !tweetedWords.get(index).equals(mostPopular))
				{
					currentPopularity++;
				}
			}
	...
	public int euroQuery(String searchName)
	{
		...
		Query euroQuery = new Query(searchName);
		...
		while (tweets < max)
		{
			try
			{
				...
				tweets += result.getTweets().size();
				for (Status currentTweet : result.getTweets())
				{
					if (currentTweet.getId() < last)
					{
						last = currentTweet.getId();
					}
	...
	public int USQuery(String searchName)
	{
		...
		USQuery.setGeoCode(new GeoLocation(38.725814, -95.564685), 1400, Query.MILES);
	...
	public String investigateSanderson()
	{
		...
		int euroCosmere = 0;// euroQuery("Cosmere");
		int euroArchive = 0;// euroQuery("Stormlight Archive");
		int euroMisborn = 0;// euroQuery("Mistborn");
		int euroAlcatraz = 0;// euroQuery("Alcatraz Versus the Evil
								// Librarians");

		int USAuthor = USQuery("Brandon Sanderson");
		int USArchive = 0;// USQuery("Stormlight Archive");
		int USCosmere = 0;// USQuery("Cosmere");
		int USMisborn = 0;// USQuery("Mistborn");
		int USAlcatraz = 0;// USQuery("Alcatraz Versus the Evil Librarians");

		int USResults = (USAuthor + USArchive + USCosmere + USMisborn + USAlcatraz);
		...
		{
			outcome += results + " Brandon Sanderson is more popular in the United States by: "
					+ (USResults - euroResults);
